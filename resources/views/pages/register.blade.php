    ...
    <div class="max-w-7xl mx-auto px-6 grid lg:grid-cols-12 gap-10">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-xl font-semibold tracking-wide">5601705447</p>
                        </div>
                        <button onclick="copyText('5601705447')" class="px-3 py-2 text-sm rounded-lg border hover:bg-gray-50">Copy</button>
                    </div>
                </div>
            </div>
        ...
        <div class="lg:col-span-7">
            <div class="card p-8 md:p-10">
                ...
                @if(session('success'))
                    <div class="mb-6 p-4 rounded-xl bg-green-50 text-green-700 border border-green-100">
                        {{ session('success') }}
                    </div>
                @endif
                ...
                <form id="multiStepForm" action="{{ route('register.delegate.store') }}" method="POST" enctype="multipart/form-data" novalidate>
                    ...
                    <div class="flex items-center justify-between mb-8">
                        <div class="flex items-center gap-3">
                            <div id="step1-indicator" class="w-8 h-8 flex items-center justify-center rounded-full {{ $errors->any() ? 'bg-red-500' : 'bg-[#1a4228]' }} text-white text-sm font-bold">1</div>
                            <span class="text-sm font-medium text-gray-700">Information</span>
                        </div>
                        <div class="flex-1 h-[1px] bg-gray-200 mx-4"></div>
                        <div class="flex items-center gap-3">
                            <div id="step2-indicator" class="w-8 h-8 flex items-center justify-center rounded-full bg-gray-200 text-gray-500 text-sm font-bold">2</div>
                    ...
                    <div id="step2" class="{{ $errors->has('payment_proof') ? '' : 'hidden' }} space-y-6">
                        ...
                        <div class="flex gap-4">
                            <button type="button" onclick="prevStep()" class="w-1/2 py-3 rounded-xl border text-gray-600 hover:bg-gray-50">Back</button>
                            <button type="submit" class="w-1/2 py-3 rounded-xl text-white font-semibold bg-gradient-to-r from-[#1a4228] to-[#14532d]">Submit Registration</button>
                        </div>
                    </div>
                </form>
            </div>
